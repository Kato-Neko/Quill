...
import { Search, ArrowLeft, Archive, Trash2, Star, MoreVertical, Edit3, Pin, StarOff, FileText, CheckSquare } from "lucide-react"
...
export default function ArchivePage() {
  ...
  const fetchArchivedNotes = async () => {
    ...
    try {
      let url = `${API_BASE_URL}/notes?size=100&archived=true`
      
      // If there's a search query, use the /search endpoint
      if (searchQuery.trim()) {
        url = `${API_BASE_URL}/notes/search?query=${encodeURIComponent(searchQuery.trim())}&size=100&archived=true`
      }
      
      const response = await fetch(url)
      
      if (!response.ok) {
        throw new Error(`Failed to fetch archived notes: ${response.status}`)
      }
  ...
  const fetchArchivedTodoLists = async () => {
    setLoading(true)
    setError(null)

    try {
      let url = `${API_BASE_URL}/todo-lists?archived=true&size=50&sort=updatedAt,desc`
      ...
      setError(err.message)
      console.error('Error fetching archived todo lists:', err)
      setTodoLists([])
    } finally {
      setLoading(false)
    }
  ...
  const handleUnarchive = async (itemId) => {
    try {
      const response = await fetch(`${API_BASE_URL}/${activeTab === "notes" ? "notes" : "todo-lists"}/${itemId}/unarchive`, {
        method: 'POST'
      })
    ...
    return (
      <div className="flex h-screen bg-background items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Loading archived {activeTab}...</p>
        </div>
      </div>
    )
  }
                ...
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {pinnedNotes.map((note) => (
                      <NoteCard key={note.id} note={note} onDelete={handleDeleteClick} onPinToggle={handlePinToggle} onStarToggle={handleStarToggle} onUnarchive={handleUnarchive} />
                    ))}
                  </div>
                </div>
              )}
                ...
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {pinnedTodoLists.map((todoList) => (
                      <TodoListCard key={todoList.id} todoList={todoList} onDelete={handleDeleteClick} onPinToggle={handlePinToggle} onStarToggle={handleStarToggle} onArchive={handleUnarchive} />
                    ))}
                  </div>
                </div>
              )}
          ...
          {loading && ((activeTab === "notes" && notes.length === 0) || (activeTab === "todos" && todoLists.length === 0)) ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium mb-2 text-foreground">
                {searchQuery ? `No archived ${activeTab} found` : `No archived ${activeTab} yet`}
              </h3>
              <p className="text-muted-foreground mb-4">
                {searchQuery 
                  ? `No archived ${activeTab} match "${searchQuery}". Try different keywords.` 
                  : `${activeTab === "notes" ? "Notes" : "Todo lists"} you archive will appear here.`
                }
              </p>
            </div>
          ) : null}
...
function NoteCard({ note, onDelete, onPinToggle, onStarToggle, onUnarchive }) {
