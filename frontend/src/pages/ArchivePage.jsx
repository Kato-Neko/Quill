...
import { Search, ArrowLeft, Archive, FileText } from "lucide-react"
...
export default function ArchivePage() {
  ...
  // Removed fetchArchivedTodoLists
  ...
  const handleUnarchive = async (itemId) => {
    try {
      const response = await fetch(`${API_BASE_URL}/notes/${itemId}/unarchive`, {
        method: 'POST'
      })
          ...
          {loading && notes.length === 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">
                {searchQuery ? `No archived notes match "${searchQuery}". Try different keywords.` : `Notes you archive will appear here.`}
              </p>
            </div>
          ) : null}
        </div>
...
// unified NoteCard used above; per-page behavior provided via props
